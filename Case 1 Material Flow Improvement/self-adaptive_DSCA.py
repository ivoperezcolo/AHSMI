...
class MH_opt(Material_Handling):
    ...
    def process_simulation(self, r, pop_size, max_iter,
                           prob, alfa, gamma, epsilon):
        if (self.connection()):
            self.connect.setvisible(False)
            route, Convergence_curve, Q, r1_curves, D_score = self.method.DSCAQL(r, pop_size, max_iter,
                                                                                 prob, alfa, gamma, epsilon)
            self.connect.closemodel()
            self.connect.quit()
        return route, Convergence_curve, Q, r1_curves, D_score
...
class MethodB(MethodA):
    ...
    def DSCAQL(self, r, Pop_size, Max_iter, prob, alfa, gamma, epsilon):
        ...
        # Initialize the Q-Tables (Actions: 4 func para r1. States: 5 (valores
        # entre 0 y 1), dependiente de F)
        Q1 = np.zeros((5, 4))
        Q2 = np.zeros((5, 4))
        Q3 = np.zeros((5, 4))
        Q4 = np.zeros((5, 4))
        ...
        for m in range(1, Max_iter):
            ...
            for i in range(Pop_size):
                ...
                if r4 > prob:
                    ...
                    if fitness < X_fitness[i]:
                        ...
                        reward = -1
                    print("Reward", reward)
                    # Update Best_Score (Best solution)
                    if fitness < D_score:
                        D_score = fitness
                        D_pos = Pos[i].copy()
                        print("Best solution", D_score)
            ...
            writer.add_scalar('Experiment', n, now)
            writer.add_scalar('Iteration', m, now)
            writer.add_scalar('Best_score', D_score, now)
            writer.add_scalar('Epsilon', epsilon, now)
            writer.add_scalar('Population_size', Pop_size, n)
            writer.add_scalar('Total_iteration', Max_iter, n)
            writer.add_scalar('Random_probability', prob, n)
            writer.add_scalar('Alfa', alfa, n)
            writer.add_scalar('Gamma', gamma, n)
            writer.add_scalar('r1', r1, now)
            writer.add_scalar('r2', r2, now)
            writer.add_scalar('r3', r3, now)
            writer.add_scalar('r4', r4, now)

        return D_pos, Convergence_curve, Q, r1_curves, D_score
...
def plant_simulation_node(pop_size, max_iter, prob, alfa, gamma, epsilon):
    global n
    my_filename = "ModeloMH.spp"
    method = MethodB()
    plant = MH_opt(method=method, filename=my_filename)
    final_route, convergence_c, Q, r1_curves, reward = plant.process_simulation(["B11", "B21", "B41", "B81", "B91", "B111", "B121", "B141", "B151", "B191"], pop_size, max_iter, prob, alfa, gamma, epsilon)
    print("Ruta optima", final_route)
    print("Tablas-Q", Q)
    # np.savetxt('p', Q[0], delimiter=',')

    with open(f'prueba{n}.csv', 'w', encoding='UTF8', newline='') as f:
        writer = csv.writer(f)
